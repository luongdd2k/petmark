...
<HTML>
	...
	<BODY>
		...
		<div id="bgf4"><div id="bgbg" style="background-image: none;"><div id="all" style="min-height: 970px;">
			<div class="menu_title">QUẢN LÝ SẢN PHẨM</div>
			...
			<div id="product_QL">
				...
				<table class="add_p">
					<thead>
						<tr>
							...
							<td class="t_category">Danh Mục</td>
				...
				<form id="formAddProduct" action="admin/ProductManagement/AddProduct" method="POST" class="add_f" enctype='multipart/form-data'><tr><table class="add_p">
					<tbody>
						<tr>
							...
							<td class="t_category">
								<select class="p_id" name="p_add_category">
									<c:forEach var="category" items="${listCategory }">
										<c:if test = "${category.id == 'SMARTPHONE'}">
											<option value="${ category.id}" selected>${ category.name}</option>
								        </c:if>
								       	<c:if test = "${category.id ne 'SMARTPHONE'}">
							...
							<td class="t_price"><input type="text" class="p_price" id="p_add_price" name="p_add_price" value="0 ₫" autocomplete="off"></td>
							<td class="t_quantity"><input type="text" class="p_quantity" id="p_add_quantity" name="p_add_quantity" value="1" autocomplete="off"></td>
				...
				<div class="p_title_2" style="display: inline-block; float:left;">Danh sách sản phẩm đang được bán</div>
				...
				<select id="cbo_sort_product2" style="">
					...
					<option value="6" style="background: #ffe6e6">Dừng kinh doanh</option>
				</select>
				...
				<table class="show_p" style="margin-top: 30px;">
					<thead>
						<tr>
							<td class="t_image">Ảnh</td>
							<td class="t_category">Mã Sản Phẩm</td>
							<td class="t_name">Tên Sản Phẩm</td>
							<td class="t_price">Giá Bán</td>
							<td class="t_quantity">Số lượng</td>
					...
					<c:forEach var="product" items="${listProduct }">
						...
						<form action='admin/ProductManagement/UpdateProduct' method='POST' enctype='multipart/form-data' class='show_f' ><table class='show_p' >
						<tbody>
								<tr>
									...
									<td class='t_category'><input type='text' name='p_fix_id' class='p_fix_id' value='${product.getId() }' autocomplete='off'  readonly='readonly'></td>
									<td class='t_name'>
										<input type='hidden' name='formIndex' class='formIndex' value='${count }'>
										<input type='hidden' name='p_fix_image' class='p_fix_image' value='${product.getImagePath() }'>
										<input type='hidden' name='p_fix_categoryID' value='${product.getCategory().getId() }'>
										<input type='hidden' name='p_fix_ori' value='${product.getPrice() }'>
										<input type='text' name='p_fix_name' class='p_fix_name' value='${product.getName() }' autocomplete='off' required>
										<a target='_blank' href='ViewProduct/${product.getId() }' class='p_more'>Xem chi tiết</a>
										<div class="countVLC">
									...
									<td class='t_price'>
										<input type='text' class='p_price' name='p_fix_ori2' value='${product.getDisplayPrice(1) }'>
										
										<div class="more_price_ori">
											<b>Giá khuyến mãi</b>
											<input style="display: none;" type='text' name='p_fix_price' class='p_price p_sale' value='${product.getDisplayPriceSale() }' autocomplete='off'>
											<input style="display: none;" type='text' name='p_hidden2' class='p_saleHidden' value='${product.getDisplayPriceSale() }' autocomplete='off'>
											<i title="Hủy bỏ khuyến mãi" style="display: none;" class="fas fa-times cancel_sale"></i>
											<div class="get_per">${product.getPer() }</div>
											<div style="display: none;" class="add_sale"><i class="fas fa-plus"></i>&nbsp;&nbsp;&nbsp;&nbsp;Thêm</div>
										</div>
										<div class="more_price_caret">
											<i class="fas fa-caret-down"></i>
										</div>
										<d	iv class="viewSaleInfo">Xem KM</div>
									</td>
									<td class='t_quantity'><input type='text' name='p_fix_quantity' class='p_quantity' value='${product.getQuantityLeft() }' autocomplete='off'></td>
									...
									<td class='t_action_2'>
										<input type='file' name='addProductIMG' class='fix_img' style='display: none'>
										<button type='button' class='${classButton2 }'><i class='fas fa-upload'></i>${nameButton2 }</button>
									</td>
									<td class='t_action_3'><button type='button' class='btn_delete ${classButtonDelete }'><i class='fas fa-trash-alt'></i>Xóa</button></td>
								</tr>
						</tbody></table></form>
					</c:forEach>
