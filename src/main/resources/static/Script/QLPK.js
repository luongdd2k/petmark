$(document).ready(function() {
	...
	$('#add_img').change(function(event) {
		isUploadIMG = true;
		$('#formAddAccessories').submit();
		$('#img_loading').css('visibility', 'visible');
		// $('#img_fff').css('visibility', 'visible');
//		$('#addAccessoriesIMGtag').css('opacity', '0.7');
		$('#addAccessoriesIMGtag').css('visibility', 'hidden');		
	});
	...
	$('#formAddAccessories').submit(function(event) {
		if(isUploadIMG==true){
...
//			var formData = new FormData($(this));
			...
			.done(function(img_resp) {
				...
				if(photo_name=='FAIL'){
					...
					$('#addAccessoriesIMGtag').css('visibility', 'visible');	
					...
					$('#addAccessoriesIMGtag').attr('src', photo_name);
			
					var srcIMG = $('#addAccessoriesIMGtag').attr('src');
					var statusIMG = false;
					var countIMG = 0;
					
					var refreshIntervalId =  setInterval(function(){ 
		 				console.log(countIMG);
						$('#addAccessoriesIMGtag').attr('src', srcIMG + '?' + new Date());
						
						$("<img/>")
					    .on('load', function() { statusIMG = true; })
					    .on('error', function() { statusIMG = false; })
					    .attr("src", $('#addAccessoriesIMGtag').attr("src"))
						;
						...
						if(statusIMG==true || countIMG==50) {
							...
							$('#addAccessoriesIMGtag').css('opacity', '1');
							$('#addAccessoriesIMGtag').css('visibility', 'visible');	
...
//		$('.fix_img').parent().parent().parent().parent().parent('.show_f').submit();
		...
		$('.updateAccessoriesIMGtag:eq('+formIndex+')').css('visibility', 'hidden');
	});
		...
		if(isUploadIMG2==true){
			...
			.done(function(img_resp) {
				...
				if(photo_name=='FAIL'){
					...
					$('.updateAccessoriesIMGtag:eq('+formIndex+')').css('visibility', 'visible');
					Confirm2('Thông báo','File không hợp lệ!<br>Vui lòng chọn file hình ảnh jpeg/png','Đồng ý','Quay lại');
				} else if (photo_name=='OVERSIZE'){
					console.log('fail OVERSIZE');
					$('.img_loading2:eq('+formIndex+')').css('visibility', 'hidden');
					...
					$('.updateAccessoriesIMGtag:eq('+formIndex+')').attr('src', photo_name);
		
					var srcIMG = $('.updateAccessoriesIMGtag:eq('+formIndex+')').attr('src');
					var statusIMG = false;
					var countIMG = 0;
					
					var refreshIntervalId =  setInterval(function(){ 
		 				console.log(countIMG);
						$('.updateAccessoriesIMGtag:eq('+formIndex+')').attr('src', srcIMG + '?' + new Date());
						
						$("<img/>")
					    .on('load', function() { statusIMG = true; })
					    .on('error', function() { statusIMG = false; })
					    .attr("src", $('.updateAccessoriesIMGtag:eq('+formIndex+')').attr("src"))
						;
						...
						if(statusIMG==true || countIMG==50) {
							...
							clearInterval(refreshIntervalId);
							$('.updateAccessoriesIMGtag:eq('+formIndex+')').css('opacity', '1');
							$('.updateAccessoriesIMGtag:eq('+formIndex+')').css('visibility', 'visible');
	...
	function ConfirmUpdate(title, msg, $true, $false, index, AccessoriesId) { /*change*/
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
			...
			var data = new FormData(form);
			$.ajax({
				url: 'admin/AccessoriesManagement/UpdateAccessories',
				type: 'POST',
				enctype: 'multipart/form-data',
				dataType: 'html',
				data: data,
			...
			.done(function(total_resp) {
				...
				console.log("Success Update Accessories!");
	...
	$('.btn_save').click(function(event) {
		...
		var AccessoriesName = $('.p_fix_name:eq('+formIndex+')').val();
		var AccessoriesId = $('.p_fix_id:eq('+formIndex+')').val();

		
			ConfirmUpdate('⚠️ Thay đổi giá trị', 
			    	"Cập nhật giá trị mới cho sản phẩm này?<br>"+
			    	"Tên SP: <a  href='ViewAccessories/"+AccessoriesId+"' class='confirmD_nameAccessories' target='_blank'>"+AccessoriesName+" ➯</a><br>"+
			    	"Xác nhận?",
			    	 				'Đồng ý', 'Quay lại', formIndex, AccessoriesId);
	...
	function ConfirmDiscontinue(title, msg, $true, $false, index, AccessoriesId) { /*change*/
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
	        $.ajax({
				url: 'admin/AccessoriesManagement/DisContinuedAccessories',
				type: 'POST',
				dataType: 'html',
				data: {
			...
			.done(function(total_resp) {
				...
				console.log("Success DisContinuedAccessories!");
				console.log(total_resp);
			})
			.fail(function() {
				console.log("error");
	...
	$('.btn_delete2').click(function(event) {
		var formIndex = $(this).parent('.t_action_3').siblings('.tenthucung').val();
		var AccessoriesName = $('.p_fix_name:eq('+formIndex+')').val();
		var AccessoriesId = $('.idthu:eq('+formIndex+')').val();
	...
	function ConfirmContinue(title, msg, $true, $false, index, AccessoriesId) { /*change*/
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
	        $.ajax({
				url: 'admin/AccessoriesManagement/ContinuedAccessories',
				type: 'POST',
				dataType: 'html',
				data: {
					"AccessoriesId": AccessoriesId
			...
			.done(function(total_resp) {
				...
				console.log("Success ContinuedAccessories!");
				console.log(total_resp);
			})
			.fail(function() {
				console.log("error");
	...
	function ConfirmPermanentlyDeleted(title, msg, $true, $false, index, AccessoriesId) {
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
	        $.ajax({
				url: 'admin/AccessoriesManagement/PermanentlyDeleted',
				type: 'POST',
				dataType: 'html',
				data: {
					"AccessoriesId": AccessoriesId
	...
	$('#cbo_sort_Accessories2').change(function(event) {

		$(location).attr('href', 'admin/AccessoriesManagement/changeSortValue?sortValue='+$(this).val());

	});
	try {
		$('#cbo_sort_Accessories2 option[value='+$('#sortValue2').text()+']').attr('selected','selected');
	} catch(e) {}
	...
	$('#qlsp_forward').click(function(event) {
		...
		if($('#qlsp_current_page').val()*1<$('#qlsp_total_page').text()*1){
			$(location).attr('href', 'admin/AccessoriesManagement?targetPage='+ targetPage +
									'&sortValue='+$('#cbo_sort_Accessories2').val());
		}
	...
	function ConfirmAddAccessories(title, msg, $true, $false) { 
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	...
	$('.btn_add').click(function(event) {
		event.preventDefault();
		ConfirmAddAccessories('⚠️ Thông báo', 
