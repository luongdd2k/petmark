...
//		$('#addProDuctIMGtag').css('opacity', '0.7');
	...
	$('#formAddSpecies').submit(function(event) {
		if(isUploadIMG==true){
...
//			var formData = new FormData($(this));
			...
			.done(function(img_resp) {
				...
				if(photo_name=='FAIL'){
					...
					$('#addSpeciesIMGtag').css('visibility', 'visible');	
					...
					$('#addSpeciesIMGtag').attr('src', photo_name);
			
					var srcIMG = $('#addSpeciesIMGtag').attr('src');
					var statusIMG = false;
					var countIMG = 0;
					
					var refreshIntervalId =  setInterval(function(){ 
		 				console.log(countIMG);
						$('#addSpeciesIMGtag').attr('src', srcIMG + '?' + new Date());
						
						$("<img/>")
					    .on('load', function() { statusIMG = true; })
					    .on('error', function() { statusIMG = false; })
					    .attr("src", $('#addSpeciesIMGtag').attr("src"))
						;
						...
						if(statusIMG==true || countIMG==50) {
							...
							$('#addSpeciesIMGtag').css('opacity', '1');
							$('#addSpeciesIMGtag').css('visibility', 'visible');	
...
//		$('.fix_img').parent().parent().parent().parent().parent('.show_f').submit();
	...
	function ConfirmUpdate(title, msg, $true, $false, index, speciesId) { /*change*/
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
			...
			var data = new FormData(form);
			$.ajax({
				url: 'admin/SpeciesManagement/UpdateSpecies',
				type: 'POST',
				enctype: 'multipart/form-data',
				dataType: 'html',
				data: data,
			...
			.done(function(total_resp) {
				...
				console.log("Success Update Species!");
	...
	$('.btn_save').click(function(event) {
		...
		var speciesName = $('.p_fix_name:eq('+formIndex+')').val();
		var speciesId = $('.p_fix_id:eq('+formIndex+')').val();

		
			ConfirmUpdate('⚠️ Thay đổi giá trị', 
			    	"Cập nhật giá trị mới cho sản phẩm này?<br>"+
			    	"Tên Species: <a  href='ViewSpecies/"+speciesId+"' class='confirmD_nameSpecies' target='_blank'>"+speciesName+" ➯</a><br>"+
			    	"Xác nhận?",
			    	 				'Đồng ý', 'Quay lại', formIndex, speciesId);
	});
	////////////////////////////////////////////////

		function ConfirmDiscontinue(title, msg, $true, $false, index, speciesId) { /*change*/
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
	        $.ajax({
				url: 'admin/SpeciesManagement/DisContinuedSpecies',
				type: 'POST',
				dataType: 'html',
				data: {
					"speciesId": speciesId
			...
			.done(function(total_resp) {
				...
				console.log("Success DisContinuedSpecies!");
				console.log(total_resp);
			})
			.fail(function() {
				console.log("error");
	...
	$('.btn_delete2').click(function(event) {
		...
		var speciesId = $('.p_fix_id:eq('+formIndex+')').val();
			...
			ConfirmDiscontinue('⚠️ Dừng bán giống', 
			    	...
			    	"Tên giống thú cưng: <a  href='ViewSpecies/"+speciesId+"' class='confirmD_nameSpecies' target='_blank'>"+speciesName+" ➯</a><br>"+
			    	"Xác nhận?",
			    	 				'Đồng ý', 'Quay lại', formIndex, speciesId);
		
	});
	
	function ConfirmContinue(title, msg, $true, $false, index, speciesId) { /*change*/
		var $content =  "<div class='dialog-ovelay'></div>" +
						"<div class='dialog'>" +
						    "<header class='headerConfirm'>" +
						        "<h3 class='h3Confirm'> " + title + " </h3> " +
	    ...
	    $('.doAction').click(function() {
	        $.ajax({
				url: 'admin/SpeciesManagement/ContinuedSpecies',
				type: 'POST',
				dataType: 'html',
				data: {
					"speciesId": speciesId
			...
			.done(function(total_resp) {
				...
				console.log("Success ContinuedSpecies!");
				console.log(total_resp);
			})
			.fail(function() {
				console.log("error");
	...
	$('.cancel_discontinue').click(function(event) {
		...
		var speciesId = $('.p_fix_id:eq('+formIndex+')').val();
			...
			ConfirmContinue('⚠️ Đăng bán sản phẩm', 
			    	...
			    	"Tên SP: <a  href='ViewProduct/"+speciesId+"' class='confirmD_nameProduct' target='_blank'>"+productName+" ➯</a><br>"+
			    	"Xác nhận?",
			    	 				'Đồng ý', 'Quay lại', formIndex, speciesId);
	...
	$('#cbo_sort_species2').change(function(event) {

		$(location).attr('href', 'admin/SpeciesManagement/changeSortValue?sortValue='+$(this).val());

	});
	try {
		$('#cbo_sort_species2 option[value='+$('#sortValue2').text()+']').attr('selected','selected');
	} catch(e) {}
	...
	$('#qlsp_forward').click(function(event) {
		...
		if($('#qlsp_current_page').val()*1<$('#qlsp_total_page').text()*1){
			$(location).attr('href', 'admin/SpeciesManagement?targetPage='+ targetPage +
									'&sortValue='+$('#cbo_sort_species2').val());
		}
